...
import { GitBranch, ShieldAlert, CheckSquare, Sparkles, Building2, ChevronRight, CheckCircle2, AlertTriangle, ArrowRightLeft, Cpu, Clock, ShieldCheck } from 'lucide-react';
    ...
    <div className="flex flex-col gap-4" id="routing-screen-layout">
      ...
      <div className="bg-white border border-[#e1e6eb] rounded-lg p-3.5 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 max-w-2xl font-medium">
            Quotes qualified in Stage 1 flow here. Verify complexity rules and dispatch to either the autonomous AI agent or human engineers.
          </p>
        </div>
      ...
      <div className="bg-slate-50 border border-[#e1e6eb] rounded-lg p-3.5 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex gap-2.5 items-start">
          ...
          <div>
            <span className="text-[10.5px] font-bold text-slate-800 uppercase block tracking-wider">Class 1: Simple Spare Parts (AI Eligible)</span>
            <span className="text-[10px] text-slate-500 font-medium leading-relaxed block mt-0.5">
              Basic casing seals, routine gaskets, and standard impellers with 100% confidence. Eligible for instant autonomous AI parsing and SAP CPQ compiling.
            </span>
          </div>
        </div>
        <div className="flex gap-2.5 items-start border-t md:border-t-0 md:border-l border-slate-200 pt-3 md:pt-0 md:pl-4">
          ...
          <div>
            <span className="text-[10.5px] font-bold text-slate-800 uppercase block tracking-wider">Class 2: Engineered Packages (Manual Desk)</span>
            <span className="text-[10px] text-slate-500 font-medium leading-relaxed block mt-0.5">
              Heavy metallurgic impellers, custom dynamic stress ratings, or structural casing trims. Requires specialist certification & manual pricing.
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white border border-[#e1e6eb] rounded-lg p-3 flex flex-col" id="col-needs-routing">
          <div className="flex items-center justify-between mb-3 px-1 border-b border-slate-100 pb-2">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="font-sans font-bold text-slate-850 text-xs uppercase tracking-wider">Qualified Backlog</h3>
            </div>
          ...
          <div className="flex flex-col gap-3" id="backlog-cards">
            {backlogQuotes.length === 0 ? (
              ...
              backlogQuotes.map((quote) => {
                const isSimple = quote.suggestedComplexity === 'Simple' && quote.id !== 'REQ-2026-006';
                return (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="text-[9px] text-slate-400 font-bold font-mono">{quote.id}</div>
                        <h4 className="font-bold text-slate-900 text-xs leading-tight">{quote.customerName}</h4>
                        <p className="text-[10px] text-slate-500 truncate max-w-[150px] font-medium">{quote.subject}</p>
                      </div>
                    ...
                    <div className="bg-slate-50 rounded-md p-2 grid grid-cols-2 gap-1.5 font-mono text-[9px] text-slate-600 border border-slate-100/50">
                      <div>
                        <span className="text-slate-450 text-[8px] block uppercase font-sans font-bold">Part Ref:</span>
                        <span className="font-bold text-slate-850 truncate block">{quote.partNumberCorrected || 'N/A'}</span>
                      </div>
                      <div>
                        <span className="text-slate-450 text-[8px] block uppercase font-sans font-bold">Serial Ref:</span>
                        <span className="font-bold text-slate-850 truncate block">{quote.serialNumberCorrected || 'N/A'}</span>
                      </div>
                    </div>
        ...
        <div className="bg-white border border-[#e1e6eb] rounded-lg p-3 flex flex-col" id="col-eng-routing">
          <div className="flex items-center justify-between mb-3 px-1 border-b border-slate-100 pb-2">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="font-sans font-bold text-slate-850 text-xs uppercase tracking-wider">Local Back Office</h3>
            </div>
              ...
              aiQuotes.map((quote) => (
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-500 bg-slate-50 p-2 rounded-md border-l-2 border-emerald-500 leading-snug">
                    🤖 Autonomous run successful. Quote compilation cached & synchronized in SAP CPQ.
                  </p>
        ...
        <div className="bg-white border border-[#e1e6eb] rounded-lg p-3 flex flex-col" id="col-hub-routing">
          <div className="flex items-center justify-between mb-3 px-1 border-b border-slate-100 pb-2">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="font-sans font-bold text-slate-850 text-xs uppercase tracking-wider">Pune Hub (Manual)</h3>
            </div>
